...
class SpectrogramParser(AudioParser):
    def __init__(self, audio_conf, normalize=False, augment=False, pitch=False,whitenoise=False):
        """
    ...
    def parse_audio(self, audio_path):
        ...
        #add White noise, shift sound, stretching sound
        activeflag = random.uniform(0, 1)
        if self.pitch and activeflag > 0.5:
            step = (np.random.rand()-0.5)*20 
            y = librosa.effects.pitch_shift(y, 16000, n_steps=step)
        activeflag = random.uniform(0, 1)
        if self.whitenoise and activeflag > 0.5:
...
class SpectrogramDataset(Dataset, SpectrogramParser):
    def __init__(self, audio_conf, manifest_filepath, labels, normalize=False, augment=False, pitch=False, whitenoise=False):
        """
